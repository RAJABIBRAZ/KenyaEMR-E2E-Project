...
The helper does not prove that a patient is synthetic merely because a caller supplied a `QAE2E-` fixture key. Registration and fixture tests must independently verify they are acting on an approved synthetic record and authorized QA environment. The existing UAT account was originally supplied for read-only performance auditing; one later registration was separately authorized. This tracker does not authorize further clinical or registration writes.
...
This is partial coverage of workbook `03_Registration!R20` / TC059, not evidence that the full Complete Registration Workflow passed. One authorized live run on 2026-09-15 observed a successful UAT save and assigned ID; no visit was started. Do not update the full workbook result to Passed based on this scoped run.

Before any additional live run, obtain explicit authorization to create synthetic patients in UAT, a suitable write-enabled QA account, and the approved session location UUID if prompted. Reuse of the audit credential file requires separate explicit approval and `QA_E2E_ALLOW_AUDIT_ACCOUNT_FOR_REGISTRATION=true`. Keep `KENYAEMR_URL`, `KENYAEMR_USERNAME`, and `KENYAEMR_PASSWORD` in an owner-only (`chmod 600`) file outside Git. The URL must target `https://uat.kenyahmis.org/openmrs/spa/login` or the same UAT origin/context. Do not put credentials in shell history or this repository. Then, only when approved:
